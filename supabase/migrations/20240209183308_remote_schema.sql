...
CREATE TYPE "public"."habit_trait" AS ENUM (
    'good',
    'bad'
);

ALTER TYPE "public"."habit_trait" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."habits" (
    ...
    "name" text NOT NULL,
    "description" text,
    "trait" public.habit_trait NOT NULL,
    "icon_path" text
);
...
ALTER TABLE ONLY "public"."calendar_events"
    ADD CONSTRAINT "calendar_events_habit_id_fkey" FOREIGN KEY (habit_id) REFERENCES public.habits(id) ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE ONLY "public"."calendar_events"
    ADD CONSTRAINT "calendar_events_user_id_fkey" FOREIGN KEY (user_id) REFERENCES public.accounts(id) ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE ONLY "public"."habits"
    ADD CONSTRAINT "habits_user_id_fkey" FOREIGN KEY (user_id) REFERENCES public.accounts(id) ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_calendar_event_id_fkey" FOREIGN KEY (calendar_event_id) REFERENCES public.calendar_events(id) ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_user_id_fkey" FOREIGN KEY (user_id) REFERENCES public.accounts(id) ON UPDATE CASCADE ON DELETE CASCADE;

CREATE POLICY "Enable delete for users based on user_id" ON "public"."calendar_events" FOR DELETE USING ((auth.uid() = user_id));
...
CREATE POLICY "Enable read access for users based on user_id" ON "public"."calendar_events" FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY "Enable update for users based on user_id" ON "public"."calendar_events" FOR UPDATE USING ((auth.uid() = user_id)) WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Users can insert their own account." ON "public"."accounts" FOR INSERT WITH CHECK ((auth.uid() = id));
